...
async def scroll_cursor_tool(direction: str, amount: int = 10):
    ...
    Useful for commands like "scroll down" or "scroll up".
...
async def type_text_tool(text: str):
    ...
    Useful for commands like "type hello world" or "type something".
...
async def press_key_tool(key: str):
    ...
    Useful for commands like "press Enter", "press Escape", or "press A".
...
async def press_hotkey_tool(keys: List[str]):
    ...
    Use this when the user says something like "save", "close window", 
    or "refresh".
...
async def control_volume_tool(action: str):
    ...
    Use this when the user says something like "volume increase", "mute", 
    or "lower the sound".
...
async def swipe_gesture_tool(direction: str):
    ...
    for example: "scroll down", "swipe left", or "swipe up".
